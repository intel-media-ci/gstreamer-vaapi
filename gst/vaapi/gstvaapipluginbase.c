...
plugin_update_sinkpad_info_from_buffer (GstVaapiPluginBase * plugin,
    GstBuffer * buf)
{
  GstVideoInfo *const vip = GST_VAAPI_PAD_INFO (plugin->sinkpad);
  GstVideoMeta *vmeta;
  guint i;
    ...
    GstDebugCategory * debug_category)
{
  ...
  /* sink pad */
  plugin->sinkpad = GST_VAAPI_SINK_PAD_CAST (gst_element_get_static_pad
      (GST_ELEMENT (plugin), "sink"));
  if (plugin->sinkpad)
    g_assert (GST_IS_VAAPI_SINK_PAD (plugin->sinkpad));

  /* src pad */
  if (!(GST_OBJECT_FLAGS (plugin) & GST_ELEMENT_FLAG_SINK))
    plugin->srcpad = GST_VAAPI_SRC_PAD_CAST (gst_element_get_static_pad
        (GST_ELEMENT (plugin), "src"));
  if (plugin->srcpad)
    g_assert (GST_IS_VAAPI_SRC_PAD (plugin->srcpad));
    ...
    guint * size)
{
  ...
  if (!reset_allocator (GST_VAAPI_PAD_ALLOCATOR (plugin->sinkpad), &vinfo))
    goto bail;
  ...
  GST_VAAPI_PAD_ALLOCATOR (plugin->sinkpad) =
      gst_vaapi_video_allocator_new (plugin->display, &vinfo, 0, usage_flag);

bail:
  if (!GST_VAAPI_PAD_ALLOCATOR (plugin->sinkpad))
    goto error_create_allocator;

  image_info =
      gst_allocator_get_vaapi_video_info (GST_VAAPI_PAD_ALLOCATOR
      (plugin->sinkpad), NULL);
    ...
    GstCaps * caps)
{
  ...
  if (!reset_allocator (GST_VAAPI_PAD_ALLOCATOR (plugin->srcpad), vinfo))
    goto valid_allocator;

  GST_VAAPI_PAD_ALLOCATOR (plugin->srcpad) = NULL;
  if (caps && gst_caps_is_video_raw (caps)) {
    GstAllocator *allocator = create_dmabuf_srcpad_allocator (plugin, vinfo,
        !plugin->srcpad->can_dmabuf);
    GST_VAAPI_PAD_ALLOCATOR (plugin->srcpad) = allocator;
  } else if (caps && gst_vaapi_caps_feature_contains (caps,
          GST_VAAPI_CAPS_FEATURE_DMABUF)) {
    GST_VAAPI_PAD_ALLOCATOR (plugin->srcpad) =
        create_dmabuf_srcpad_allocator (plugin, vinfo, FALSE);
    if (!GST_VAAPI_PAD_ALLOCATOR (plugin->srcpad))
      goto error_create_allocator;
  }
...
valid_allocator:
  ...
  if (GST_IS_VIDEO_DECODER (plugin)) {
    ...
    gboolean different_caps = GST_VAAPI_PAD_CAPS (plugin->srcpad) &&
        !gst_caps_is_strictly_equal (GST_VAAPI_PAD_CAPS (plugin->srcpad), caps);
    ...
    if (different_caps) {
      guint i;
      GstVideoInfo vi = *GST_VAAPI_PAD_INFO (plugin->srcpad);
      ...
      gst_allocator_set_vaapi_negotiated_video_info
          (GST_VAAPI_PAD_ALLOCATOR (plugin->srcpad), &vi);
...
ensure_sinkpad_buffer_pool (GstVaapiPluginBase * plugin, GstCaps * caps)
{
  ...
  if (GST_VAAPI_PAD_BUFFER_POOL (plugin->sinkpad)) {
    if (gst_vaapi_buffer_pool_caps_is_equal
        (GST_VAAPI_PAD_BUFFER_POOL (plugin->sinkpad), caps))
      return TRUE;
    gst_buffer_pool_set_active (GST_VAAPI_PAD_BUFFER_POOL
        (plugin->sinkpad), FALSE);
    g_clear_object (&GST_VAAPI_PAD_BUFFER_POOL (plugin->sinkpad));
    g_clear_object (&GST_VAAPI_PAD_ALLOCATOR (plugin->sinkpad));
    GST_VAAPI_PAD_BUFFER_SIZE (plugin->sinkpad) = 0;
  }
  ...
  GST_VAAPI_PAD_BUFFER_POOL (plugin->sinkpad) = pool;
  GST_VAAPI_PAD_BUFFER_SIZE (plugin->sinkpad) = size;
  return TRUE;
}
...
gst_vaapi_plugin_base_set_caps (GstVaapiPluginBase * plugin, GstCaps * incaps,
    GstCaps * outcaps)
{
  if (incaps && incaps != GST_VAAPI_PAD_CAPS (plugin->sinkpad)) {
    if (!gst_video_info_from_caps (GST_VAAPI_PAD_INFO
            (plugin->sinkpad), incaps))
      return FALSE;
    gst_caps_replace (&GST_VAAPI_PAD_CAPS (plugin->sinkpad), incaps);
    GST_VAAPI_PAD_CAPS_IS_RAW (plugin->sinkpad) =
        !gst_caps_has_vaapi_surface (incaps);
  }

  if (outcaps && outcaps != GST_VAAPI_PAD_CAPS (plugin->srcpad)) {
    if (!gst_video_info_from_caps (GST_VAAPI_PAD_INFO
            (plugin->srcpad), outcaps))
      return FALSE;
    if (GST_VAAPI_PAD_BUFFER_POOL (plugin->srcpad)
        &&
        !gst_vaapi_buffer_pool_caps_is_equal
        (GST_VAAPI_PAD_BUFFER_POOL (plugin->srcpad), outcaps)) {
      gst_buffer_pool_set_active (GST_VAAPI_PAD_BUFFER_POOL
          (plugin->srcpad), FALSE);
      g_clear_object (&GST_VAAPI_PAD_BUFFER_POOL (plugin->srcpad));
      g_clear_object (&GST_VAAPI_PAD_ALLOCATOR (plugin->srcpad));
      plugin_reset_texture_map (plugin);
    }
    gst_caps_replace (&GST_VAAPI_PAD_CAPS (plugin->srcpad), outcaps);
  }

  if (!ensure_sinkpad_buffer_pool (plugin,
          GST_VAAPI_PAD_CAPS (plugin->sinkpad)))
    return FALSE;
  return TRUE;
}
    ...
    GstQuery * query)
{
  ...
  gst_query_add_allocation_param (query,
      GST_VAAPI_PAD_ALLOCATOR (plugin->sinkpad), NULL);
    ...
    GstQuery * query)
{
  ...
  for (i = 0; i < num_allocators; i++) {
    ...
    if (i == 0
      ...
      if (plugin->srcpad->other_allocator)
        gst_object_unref (plugin->srcpad->other_allocator);
      plugin->srcpad->other_allocator = allocator;
      plugin->srcpad->other_allocator_params = params;
      continue;
    }
    ...
    if (g_strcmp0 (allocator->mem_type, GST_VAAPI_VIDEO_MEMORY_NAME) == 0) {
      ...
      index_allocator = i;
      if (GST_VAAPI_PAD_ALLOCATOR (plugin->srcpad))
        gst_object_unref (GST_VAAPI_PAD_ALLOCATOR (plugin->srcpad));
      GST_VAAPI_PAD_ALLOCATOR (plugin->srcpad) = allocator;
      break;
    }
    gst_object_unref (allocator);
  ...
  if (!pool) {
    ...
    pool = gst_vaapi_plugin_base_create_pool (plugin, caps, size, min, max,
        pool_options, GST_VAAPI_PAD_ALLOCATOR (plugin->srcpad));
    if (!pool)
      goto error;
  }
  ...
  g_clear_object (&GST_VAAPI_PAD_BUFFER_POOL (plugin->srcpad));
  GST_VAAPI_PAD_BUFFER_POOL (plugin->srcpad) = pool;
    ...
    GstBuffer * inbuf, GstBuffer ** outbuf_ptr)
{
  ...
  if (!GST_VAAPI_PAD_CAPS_IS_RAW (plugin->sinkpad))
    goto error_invalid_buffer;

  if (!GST_VAAPI_PAD_BUFFER_POOL (plugin->sinkpad))
    goto error_no_pool;

  if (!gst_buffer_pool_is_active (GST_VAAPI_PAD_BUFFER_POOL (plugin->sinkpad))
      &&
      !gst_buffer_pool_set_active (GST_VAAPI_PAD_BUFFER_POOL
          (plugin->sinkpad), TRUE))
  ...
  outbuf = NULL;
  if (gst_buffer_pool_acquire_buffer (GST_VAAPI_PAD_BUFFER_POOL
          (plugin->sinkpad), &outbuf, NULL) != GST_FLOW_OK)
  ...
  if (!gst_video_frame_map (&src_frame,
          GST_VAAPI_PAD_INFO (plugin->sinkpad), inbuf, GST_MAP_READ))
    goto error_map_src_buffer;

  if (!gst_video_frame_map (&out_frame,
          GST_VAAPI_PAD_INFO (plugin->sinkpad), outbuf, GST_MAP_WRITE))
    ...
    GstPadDirection direction)
{
  ...
  GstCaps *out_caps;
  gboolean ret = FALSE;
  GstVaapiPad *pad = (direction == GST_PAD_SRC ? GST_VAAPI_PAD (plugin->srcpad)
      : GST_VAAPI_PAD (plugin->sinkpad));

  if (pad->allowed_raw_caps)
    return TRUE;
  ...
  gst_caps_replace (&pad->allowed_raw_caps, out_caps);
...
gst_vaapi_plugin_base_get_allowed_sinkpad_raw_caps (GstVaapiPluginBase * plugin)
{
  if (!ensure_allowed_raw_caps (plugin, GST_VIDEO_FORMAT_UNKNOWN, GST_PAD_SINK))
    return NULL;
  return GST_VAAPI_PAD_ALLOWED_RAW_CAPS (plugin->sinkpad);
}
    ...
    plugin, GstVideoFormat format)
{
  if (!ensure_allowed_raw_caps (plugin, format, GST_PAD_SRC))
    return NULL;
  return GST_VAAPI_PAD_ALLOWED_RAW_CAPS (plugin->srcpad);
}
    ...
    GstBuffer * inbuf, GstBuffer * outbuf)
{
  ...
  if (!gst_video_frame_map (&src_frame,
          GST_VAAPI_PAD_INFO (plugin->srcpad), inbuf, GST_MAP_READ))
    return FALSE;
  if (!gst_video_frame_map (&dst_frame,
          GST_VAAPI_PAD_INFO (plugin->srcpad), outbuf, GST_MAP_WRITE)) {
    gst_video_frame_unmap (&src_frame);
    return FALSE;
  }
