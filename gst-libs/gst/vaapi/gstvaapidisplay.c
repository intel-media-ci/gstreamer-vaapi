...
struct _GstVaapiProfileConfig
{
  ...
  gboolean entrypoint[GST_VAAPI_MAX_ENTRYPOINTS];
};
...
ensure_profiles (GstVaapiDisplay * display)
{
  ...
  for (i = 0; i < n; i++) {
    ...
    for (j = 0; j < num_entrypoints; j++)
      config.entrypoint[gst_vaapi_entrypoint (entrypoints[j])] = TRUE;
  ...
  for (i = 0; i < priv->codecs->len; i++) {
    ...
    if (codec->entrypoint[GST_VAAPI_ENTRYPOINT_VLD]
        || codec->entrypoint[GST_VAAPI_ENTRYPOINT_IDCT]
        || codec->entrypoint[GST_VAAPI_ENTRYPOINT_MOCO])
      g_ptr_array_add (priv->decoders, codec);
    if (codec->entrypoint[GST_VAAPI_ENTRYPOINT_SLICE_ENCODE]
        || codec->entrypoint[GST_VAAPI_ENTRYPOINT_PICTURE_ENCODE]
        || codec->entrypoint[GST_VAAPI_ENTRYPOINT_SLICE_ENCODE_LP]
        || codec->entrypoint[GST_VAAPI_ENTRYPOINT_SLICE_ENCODE_FEI])
