...
static const GstVideoFormatMap gst_vaapi_video_default_formats[] = {
  /* The define of LSB and MSB of video format is a confused and ambigious.
     For MSB, the format have no ambiguity, same order in define, memory
     and CPU. For example, RGBA is R G B A in memory and R G B A with
     channel mask R:0xFF0000 G:0x00FF0000 B:0x0000FF00 A:0x000000FF in CPU.
     For LSB, CPU's perspective and memory's perspective generate different
     result. For example, the RGBA in LSB, from CPU's perspective, it's RGBA
     order in memory, and when load it from memory, because the little
     endianness of CPU, it will be re-ordered in register, with mask
     R:0x000000ff G:0x0000ff00 B:0x00ff0000 A:0xff000000.
     While From memory's perspective, RGBA LSB in define is equal to ABGR MSB
     in memory.
     The define is mixed used all over the media system and we need to correct
     map the va video format to gst video format in both manners, especially
     for RGB format. */
  /* YUV formats */
