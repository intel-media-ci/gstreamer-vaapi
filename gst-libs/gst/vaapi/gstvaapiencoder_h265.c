...
ensure_tile (GstVaapiEncoderH265 * encoder)
{
  ...
  /* ensure not expand max supported slices */
  ...
  /* Then divide CTUs in one tile uniformly to each slice. Note: the slice start
     address is calculate in tile mode, that is, we accumulate all CTUs in tile0,
     then tile1, and tile2..., not from the picture's perspective. */
