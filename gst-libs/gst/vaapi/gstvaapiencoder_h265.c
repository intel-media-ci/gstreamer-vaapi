...
ensure_tier_level (GstVaapiEncoderH265 * encoder)
{
  ...
  /* promote the level by tile setting */
  if (h265_is_tile_enabled (encoder)) {
    ...
    if (i == num_limits) {
      GST_WARNING
          ("can not find level for num-tile-cols is %d, num-tile-rows %d",
    ...
    GstVaapiCodedBuffer * codedbuf, GstVaapiSurfaceProxy * surface)
{
  ...
  if (pic_param->pic_fields.bits.tiles_enabled_flag) {
    ...
    for (i = 0; i < MIN (encoder->num_tile_cols, 19); ++i)
      pic_param->column_width_minus1[i] = encoder->tile_ctu_cols[i] - 1;
    for (i = 0; i < MIN (encoder->num_tile_rows, 21); ++i)
      pic_param->row_height_minus1[i] = encoder->tile_ctu_rows[i] - 1;
  }
...
add_one_slice (GstVaapiEncoderH265 * encoder, GstVaapiEncPicture * picture,
    ...
    GstVaapiEncoderH265Ref ** reflist_1, guint reflist_1_count)
{
  ...
  if (h265_is_tile_enabled (encoder)) {
    for (i_slice = 0; i_slice < encoder->num_slices; ++i_slice) {
      ...
      slice = add_one_slice (encoder, picture, reflist_0, reflist_0_count,
